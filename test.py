...
from cooperativegames import all_subsets, _in_set
...
def test1_shapley_value(funct):
    ...
    There are k workers w1,...,wk, each of whom contributes an amount p to the
    total profit. So N = {o, w1,...,wk} and v(S) = 0 if o is not a member of S
    and v(S) = mp if S contains the owner and m workers. Computing the Shapley
    value for this coalition game leads to a value of kp/2 for the owner and
    p/2 for each worker.
...
def test1_banzhaf_index(funct):
    ...
    [6; 4, 3, 2, 1]
    The numbers in the brackets mean a measure requires 6 votes to pass, and
    voter A can cast four votes, B three votes, C two, and D one. The winning
    groups, with parenthesis swing voters, are as follows:
...
def test2_banzhaf_index(funct):
    ...
    Consider the U.S. Electoral College. Each state has more or less power than
    the next state. There are a total of 538 electoral votes. A majority vote
    is considered 270 votes. The Banzhaf power index would be a mathematical
    ...
    The United States is having a presidential election between a Republican
    and a Democrat. For simplicity, suppose that only three states are
    participating: California (55 electoral votes), Texas (34 electoral votes),
    and New York (31 electoral votes).
...
def test4_banzhaf_index(funct):
    ...
    monopoly. The size of the market is X = 54 millions units per year (i.e.
    petroleum barrels) for a monopoly. The maximum production capacity of these
    companies is A = 44, B = 32, C = 20, D = 8 and E = 4 millions of units per
    year. Therefore, there is a set of coalitions able to provide the 54
    millions of units necessary for the monopoly, and a set of coalitions
    unable to provide that number. In each of the sufficient coalitions we may
    have necessary members (for the coalition to provide the required
    production) and unnecessary members (underlined in the table below). Even
    when one of these unnecessary members goes out of the sufficient coalition
    that coalition is able to provide the required production. However, when
    one necessary member leaves, the sufficient coalition becomes insufficient.
    The monopoly's profit to be distributed among the coalition's members is
    100 millions of dollars per year.

    A is necessary in 38,5% of the total cases, B in 23.1%, C in 23.1%, D in
    7.7% and E in 7.7% (these are the Banshaf indexes for each company).
    """
...
def test5_banzhaf_index(funct):
    ...
    Treaty_of_Rome_.281958.E2.80.931973.29)
    """
